...
- 关于线程局部变量。sylar的每个线程都有两个线程局部变量，一个用于存储当前线程的Thread指针，另一个存储线程名称，通过Thread::GetCurrent()可以拿到当前线程的指针。一般都要求线程有名称
...
实现协程调度之后，可以解决协程模块中子协程不能直接运行另一个子协程的缺陷，子协程可以通过向调度器添加调度任务的方式来运行另一个子协程。
