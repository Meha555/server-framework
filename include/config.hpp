 ...
 * NOTE 而我们需要转换自定义的类型，可以选择实现对应类型的流运算符，或者将该转换函数封装一个仿函数模板并进行偏特化，并重写提供自己的operator()，内含流运算符逻辑。
...
public:
    ...
    std::string toString() const override
    {
        try {
            ...
            LOG_FMT_ERROR(GET_ROOT_LOGGER(), "ConfigItem::toString exception %s in convert: %s to string", e.what(), typeid(m_value).name());
        }
        return "<error>";
    }
    // 将字符串反序列化为配置项的值
    bool fromString(const std::string &val) override
    {
        try {
            ...
            LOG_FMT_ERROR(GET_ROOT_LOGGER(), "ConfigItem::fromString exception %s in convert: string to %s", e.what(), typeid(m_value).name());
        }
        return false;
    }
...
public:
    ...
    static typename ConfigItem<T>::sptr Lookup(const std::string &name)
    {
        ...
        if (!ptr) {
            LOG_FMT_ERROR(GET_ROOT_LOGGER(), "Config::Lookup<%s> exception, 无法转换到 ConfigItem<%s>", typeid(T).name(), typeid(T).name());
            throw std::bad_cast();
        }
        return ptr;
    }
    ...
    Lookup(const std::string &name, const T &value, const std::string &description = "")
    {
        ...
        if (name.find_first_not_of("qwertyuiopasdfghjklzxcvbnm0123456789._") != std::string::npos) {
            // 开头前缀不是这些字符
            LOG_FMT_ERROR(GET_ROOT_LOGGER(), "Congif::Lookup exception name=%s" "参数只能以字母数字点或下划线开头", name.c_str());
