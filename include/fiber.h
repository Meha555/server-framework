...
public:
  ...
  bool isFinished() const noexcept { return m_state == TERM; }
  bool isScheduled() const noexcept { return m_scheduled; }

private:
  // 无参构造用于创建 master fiber，设置为私有的即不允许用户创建master fiber
  explicit Fiber();

  void swapIn(Fiber* to) noexcept;
  void swapOut(Fiber* off) noexcept;
