...
//     std::make_shared<meha::LogEvent>(__FILE__, __LINE__, meha::GetThreadID(), meha::GetFiberID(), message, level)
...
public:
    ...
    struct FormatItemBase
    {
        ...
        virtual void format(std::ostream &out, const LogEvent::ptr msg) = 0;
    };

    explicit LogFormatter(const std::string &pattern);
    // 将日志事件按照构造时传入的pattern格式化成字符串
    std::string format(LogEvent::ptr msg);

private:
    // 基于有限状态机解析日志格式化模板字符串，得到由FormatItemBase子类对象组成的“元数据结构”
    void parse();
    std::string m_pattern;                       // 日志格式化字符串模板
    std::list<FormatItemBase::ptr> m_fmt_items;  // 日志格式化字符串模板解析后的得到的日志项列表
};
...
public:
    ...
    explicit LogAppender(LogEvent::LogLevel::Level level = LogEvent::LogLevel::DEBUG);
...
public:
    ...
    explicit StdoutLogAppender(LogEvent::LogLevel::Level level = LogEvent::LogLevel::DEBUG);
    // thread-safe
...
public:
    ...
    explicit FileLogAppender(const std::string &filename, LogEvent::LogLevel::Level level = LogEvent::LogLevel::DEBUG);
    // thread-safe
    void log(const LogEvent::LogLevel::Level level, const LogEvent::ptr event) override;
    bool reopen();
...
/**
 * @brief 日志器
 * 负责（同步）判断日志级别，将高于最低级别的日志用LogAppender输出器输出指定的位置
 */
class Logger {
public:
...
private:
    ...
    std::string m_pattern;                    // 日志输格式化器的默认pattern
    LogFormatter::ptr m_default_formatter;    // 日志默认格式化器，当加入 m_appenders
                                              // 的 appender 没有自己 pattern
                                              // 时，使用该 Logger 默认的的 pattern
    std::list<LogAppender::ptr> m_appenders;  // Appender列表（需要枷锁保护）
    mutable Mutex m_mutex;
};

/**
 * @brief 日志器管理器单例类
 * 用于统一管理所有的日志器，提供日志器的创建与获取方法
...
/**
 * @brief 日志事件包装类
 * @details 在日志现场的全表达式构造，包装了日志器和日志事件两个对象组合为匿名对象。
 * 在日志记录结束后，LogEventWrapper析构时，调用日志器的log方法输出日志事件。
 */
class LogEventWrapper final {
public:
    explicit LogEventWrapper(Logger::ptr logger, const LogEvent::ptr event) : m_logger(logger), m_event(event) {}
