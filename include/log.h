...
#define LOG(logger, level)                                                     \
  ...
  meha::LogEventWrapper(                                                       \
      logger, std::make_shared<meha::LogEvent>(                                \
                  __FILE__, __LINE__, __PRETTY_FUNCTION__,                     \
                  meha::utils::GetThreadID(), meha::utils::GetFiberID(),       \
                  logger->getCategory(), meha::LogEvent::LogLevel::level))     \
      .getSS()
...
struct LogEvent {
  ...
  LogEvent(const std::string &file, const uint32_t line,
      ...
      : category(category), level(level), file(file), line(line),
        function(func), thread_id(tid), fiber_id(fid),
...
struct LogAppenderConfig {
  ...
  bool operator==(const LogAppenderConfig &rhs) const {
    return type == rhs.type && level == rhs.level && pattern == rhs.pattern &&
           file == rhs.file;
  }
