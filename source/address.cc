...
sa_family_t Address::family() const { return address()->sa_family; }
...
bool Address::operator==(const Address &rhs) const { return (addrLen() == rhs.addrLen()) && !std::memcmp(address(), rhs.address(), addrLen()); }
...
IPv4Address::sptr IPv4Address::Create(const std::string &socket_str)
{
    ...
    if (pos == std::string::npos) {
        LOG_ERROR(GET_ROOT_LOGGER(), "构造完整的Socket地址必须具有IP地址和端口号");
        return nullptr;
    } else {
        ...
        addr.sin_port = std::atoi(port_str.data());
        if (ret <= 0 || addr.sin_port <= 0) {
            LOG_FMT_ERROR(GET_ROOT_LOGGER(), "IP地址或端口号解析错误: %s(%d)", std::strerror(errno), errno);
            return nullptr;
        }
...
const sockaddr *IPv4Address::address() const { return reinterpret_cast<const sockaddr *>(&m_addr); }

socklen_t IPv4Address::addrLen() const { return sizeof(sockaddr_in); }
...
IPAddress::sptr IPv4Address::broadcastAddress(uint32_t mask_bits)
{
    ...
    baddr.sin_addr.s_addr |= net::ByteSwap(net::GenMask<uint32_t>(mask_bits));
...
IPAddress::sptr IPv4Address::networkAddress(uint32_t mask_bits)
{
    ...
    naddr.sin_addr.s_addr &= ~net::ByteSwap(net::GenMask<uint32_t>(mask_bits));
...
IPAddress::sptr IPv4Address::subnetMask(uint32_t mask_bits)
{
    ...
    smask.sin_addr.s_addr = net::ByteSwap(net::GenMask<uint32_t>(mask_bits));
    return std::make_shared<IPv4Address>(smask);
}

uint32_t IPv4Address::port() const { return ntohs(m_addr.sin_port); }
...
IPv6Address::sptr IPv6Address::Create(const std::string &socket_str)
{
    ...
    if (pos == std::string::npos) {
        ...
        addr.sin6_port = std::atoi(port_str.data());
...
const sockaddr *IPv6Address::address() const { return reinterpret_cast<const sockaddr *>(&m_addr); }

socklen_t IPv6Address::addrLen() const { return sizeof(sockaddr_in6); }
...
IPAddress::sptr IPv6Address::broadcastAddress(uint32_t mask_bits)
{
    ...
    baddr.sin6_addr.s6_addr[mask_bits / 8] |= net::GenMask<uint8_t>(mask_bits % 8);
    // 2. 接着处理剩余的字节
...
IPAddress::sptr IPv6Address::networkAddress(uint32_t mask_bits)
{
    ...
    naddr.sin6_addr.s6_addr[mask_bits / 8] &= ~net::GenMask<uint8_t>(mask_bits % 8);
    // for (int i = mask_bits / 8 + 1; i < 16; ++i) {
...
IPAddress::sptr IPv6Address::subnetMask(uint32_t mask_bits)
{
    ...
    subnet.sin6_addr.s6_addr[mask_bits / 8] = net::GenMask<uint8_t>(mask_bits % 8);
...
uint32_t IPv6Address::port() const { return ntohs(m_addr.sin6_port); }
...
const sockaddr *UnixAddress::address() const { return reinterpret_cast<const sockaddr *>(&m_addr); }

socklen_t UnixAddress::addrLen() const { return m_length; }

std::string UnixAddress::path() const { return m_addr.sun_path; }
...
std::ostream &UnixAddress::marshal(std::ostream &os) const { return os << m_addr.sun_path; }
...
const sockaddr *UnknowAddress::address() const { return reinterpret_cast<const sockaddr *>(&m_addr); }
