...
FileDescriptor::FileDescriptor(int fd)
    : m_is_init(false), m_is_socket(false), m_system_non_block(false),
      m_user_non_block(false), m_is_closed(false), m_fd(fd), m_recv_timeout(-1),
      m_send_timeout(-1), m_iom(nullptr) {
  init();
}
...
bool FileDescriptor::close() { return false; }
...
FileDescriptorManagerImpl::FileDescriptorManagerImpl() { m_data.resize(64); }
...
FileDescriptor::ptr FileDescriptorManagerImpl::get(int fd, bool auto_create) {
  ...
  assert(m_data.size() > static_cast<size_t>(fd) &&
         "vector FileDescriptorManagerImpl::m_data 越界访问");
