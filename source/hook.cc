...
static ssize_t
doIO(int fd, OriginFunc func, const char *hook_func_name, uint32_t event, int fd_timeout_type, Args &&...args)
{
        ...
        if (timeout != static_cast<uint64_t>(-1)) {
            timer = iom->addConditionTimer(
                ...
                [timer_info_wp, fd, iom, event]() {
                    ...
                    iom->cancelEventListener(fd, static_cast<meha::FDEventType>(event));
                },
                timer_info_wp);
        }
        int c = 0;
    ...
    if (timeout_ms != static_cast<uint64_t>(-1)) {
        timer = iom->addConditionTimer(
            ...
            [weak_timer_info, sockfd, iom]() {
                ...
                t->cancelled = ETIMEDOUT;
                iom->cancelEventListener(sockfd, meha::FDEventType::WRITE);
            },
            weak_timer_info);
    }
...
{
    return doIO(fd, read_f, "read", meha::FDEventType::READ, SO_RCVTIMEO, buf, count);
}

ssize_t readv(int fd, const struct iovec *iov, int iovcnt)
{
    return doIO(fd, readv_f, "readv", meha::FDEventType::READ, SO_RCVTIMEO, iov, iovcnt);
}

ssize_t recv(int sockfd, void *buf, size_t len, int flags)
{
    return doIO(sockfd, recv_f, "recv", meha::FDEventType::READ, SO_RCVTIMEO, buf, len, flags);
}

ssize_t recvfrom(int sockfd, void *buf, size_t len, int flags, struct sockaddr *src_addr, socklen_t *addrlen)
{
    return doIO(
        sockfd, recvfrom_f, "recvfrom", meha::FDEventType::READ, SO_RCVTIMEO, buf, len, flags, src_addr, addrlen);
}

ssize_t recvmsg(int sockfd, struct msghdr *msg, int flags)
{
    return doIO(sockfd, recvmsg_f, "recvfrom", meha::FDEventType::READ, SO_RCVTIMEO, msg, flags);
}

ssize_t write(int fd, const void *buf, size_t count)
{
    return doIO(fd, write_f, "write", meha::FDEventType::WRITE, SO_SNDTIMEO, buf, count);
}

ssize_t writev(int fd, const struct iovec *iov, int iovcnt)
{
    return doIO(fd, writev_f, "writev_f", meha::FDEventType::WRITE, SO_SNDTIMEO, iov, iovcnt);
}

ssize_t send(int sockfd, const void *buf, size_t len, int flags)
{
    return doIO(sockfd, send_f, "send", meha::FDEventType::WRITE, SO_SNDTIMEO, buf, len, flags);
}

ssize_t sendto(int sockfd, const void *buf, size_t len, int flags, const struct sockaddr *dest_addr, socklen_t addrlen)
{
    return doIO(sockfd, sendto_f, "sendto", meha::FDEventType::WRITE, SO_SNDTIMEO, buf, len, flags, dest_addr, addrlen);
}

ssize_t sendmsg(int sockfd, const struct msghdr *msg, int flags)
{
    return doIO(sockfd, sendmsg_f, "sendmsg", meha::FDEventType::WRITE, SO_SNDTIMEO, msg, flags);
}
