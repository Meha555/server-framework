...
int IOManager::addEventListener(int fd, FDEventType event,
                                std::function<void()> callback) {
  /**
   * NOTE:
   *  主要工作流程: 首先从 fd
   * 对象池中取出对应的对象指针，如果不存在就扩容对象池， 第二步检查 fd
   * 对象是否存在相同的事件， 第三步创建 epoll 事件对象，并注册事件, 最后更新 fd
   * 对象的事件处理器
   * */
  ...
  if (fd_ctx->m_events & event) {
    LOG_FMT_ERROR(root_logger,
                  "IOManager::addEventListener 重复添加相同的事件 fd = %d, "
                  "event = %d, fd_ctx.event = %d",
   ...
   **/
  int op =
      fd_ctx->m_events == FDEventType::NONE ? EPOLL_CTL_ADD : EPOLL_CTL_MOD;
  // 创建事件
  epoll_event epevent{};
  ...
  assert(event_handler.m_scheduler == nullptr && !event_handler.m_fiber &&
...
bool IOManager::removeEventListener(int fd, FDEventType event) {
  ...
  if (!(fd_ctx->m_events & event)) { // 要移除的事件不存在
    return false;
  }
  // 从 epoll 上移除该事件的监听
    ...
    for (int i = 0; i < result; i++) {
      ...
      if (rt == -1) {
        LOG_FMT_ERROR(root_logger,
                      ...
                      m_epoll_fd, op, fd_ctx->m_fd, ev.events, rt, errno,
                      strerror(errno));
      }
      // 触发该 fd 对应的事件的处理器
...
FDContext::EventHandler &FDContext::getEventHandler(FDEventType type) {
  switch (type) {
...
void FDContext::triggerEvent(FDEventType type) {
  /**
   * NOTE: 调用调度器的 schedule 方法时，传参使用了 move 语义，等于说调用了本
   * triggerEvent 方法后， 会把处理对应事件的协程或函数对象的引用从本 FDContext
   * 对象中移除。
   * */
