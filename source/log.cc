...
{
    std::vector<LogConfig> operator()(const std::string &source) const
    {
...
{
    std::string operator()(const std::vector<LogConfig> &source) const
    {
        YAML::Node node;
...
{
    const auto iter = m_loggerMap.find("root");
    if (iter == m_loggerMap.end()) { // 日志器 map 里不存在全局日志器
        auto global_logger = std::make_shared<Logger>();
        global_logger->addAppender(std::make_shared<StdoutLogAppender>());
        m_loggerMap.insert(std::make_pair("root", std::move(global_logger)));
    ...
    for (const auto &config_log : config_log_list) {
        ...
        for (const auto &config_app : config_log.appenders) {
            ...
            case LogAppenderConfig::FILE:
                ...
                break;
            default:
                // REVIEW 这些使用std::cerr的地方，也是因为不能使用Logger模块吗？
...
Logger::sptr __LoggerManager::getLogger(const std::string &category) const
{
    ...
    const auto iter = m_loggerMap.find(category);
