 ...
 */
template <> struct lexical_cast<std::string, std::vector<LogConfig>> {
  std::vector<LogConfig> operator()(const std::string &source) {
    ...
    if (node.IsSequence()) {
      for (const auto log_config : node) {
        LogConfig lc{};
        lc.category = log_config["category"]
                          ? log_config["category"].as<std::string>()
                          : "";
        lc.level =
            log_config["level"]
                ? (LogEvent::LogLevel::Level)(log_config["level"].as<int>())
                : LogEvent::LogLevel::UNKNOWN;
        lc.pattern = log_config["pattern"]
                         ? log_config["pattern"].as<std::string>()
                         : ""; // 日志器默认格式
        if (log_config["appender"] && log_config["appender"].IsSequence()) {
          for (const auto app_config : log_config["appender"]) {
            ...
            ac.file =
                app_config["file"] ? app_config["file"].as<std::string>() : "";
            ac.level = static_cast<LogEvent::LogLevel::Level>(
                app_config["level"] ? app_config["level"].as<int>() : lc.level);
            ac.pattern = app_config["pattern"]
                             ? app_config["pattern"].as<std::string>()
                             : lc.pattern;
...
template <> struct lexical_cast<std::vector<LogConfig>, std::string> {
...
struct PlainFormatItem : public LogFormatter::FormatItemBase {
  ...
  void format(std::ostream &out, const LogEvent::sptr event) const override {
    out << m_str;
  }
...
struct LevelFormatItem : public LogFormatter::FormatItemBase {
  void format(std::ostream &out, const LogEvent::sptr event) const override {
    out << std::left << std::setw(5)
        << LogEvent::LogLevel::levelToString(event->level);
  }
};
...
// 日志模板格式控制符对应的日志项实现类
thread_local static const std::unordered_map<char,
                                             LogFormatter::FormatItemBase::sptr>
    g_format_item_map{
...
Logger::Logger()
    : m_category("default"), m_base_level(LogEvent::LogLevel::UNKNOWN),
...
Logger::Logger(const std::string &category,
    ...
    : m_category(category), m_base_level(lowest_level), m_pattern(pattern) {
...
void Logger::log(const LogEvent::sptr event) {
...
void Logger::debug(const LogEvent::sptr event) {
  event->level = LogEvent::LogLevel::DEBUG;
  log(event);
}

void Logger::info(const LogEvent::sptr event) {
  event->level = LogEvent::LogLevel::INFO;
  log(event);
}

void Logger::warn(const LogEvent::sptr event) {
  event->level = LogEvent::LogLevel::WARN;
  log(event);
}

void Logger::error(const LogEvent::sptr event) {
  event->level = LogEvent::LogLevel::ERROR;
  log(event);
}

void Logger::fatal(const LogEvent::sptr event) {
...
StdoutLogAppender::StdoutLogAppender(LogEvent::LogLevel::Level level)
    : LogAppender(level) {}

void StdoutLogAppender::log(const LogEvent::LogLevel::Level level,
                            const LogEvent::sptr event) {
...
LogAppender::LogAppender(LogEvent::LogLevel::Level level)
    : m_base_level(level) {}
...
// NOTE
// 不能关了文件，因为其他日志输出器可能也在用这个文件（需不需要管这个问题，ofs有像fd那样的引用计数吗）
...
void FileLogAppender::log(const LogEvent::LogLevel::Level level,
                          const LogEvent::sptr event) {
  if (level < m_base_level)
    return;
  ScopedLock lock(&m_mutex);
...
void LogFormatter::parse() {
  ...
  for (size_t i = 0; i < m_pattern.length(); i++) { // 注意这里每次i都会+1
    ...
    case DO_CREATE: { // 处理占位符
      assert(!g_format_item_map.empty() &&
             "g_format_item_map 没有被正确的初始化");
...
std::string LogFormatter::format(LogEvent::sptr event) const {
...
void __LoggerManager::init() {
  ...
  for (const auto &config_log : config_log_list) {
    ...
    auto logger = std::make_shared<Logger>(
        config_log.category, config_log.level, config_log.pattern);
    for (const auto &config_app : config_log.appenders) {
      ...
      case LogAppenderConfig::FILE:
        appender = std::make_shared<FileLogAppender>(config_app.file,
                                                     config_app.level);
        break;
      default:
    ...
    m_logger_map.insert(std::make_pair(config_log.category, logger));
  }
  // 确保存在一个全局的日志器
...
Logger::ptr __LoggerManager::getLogger(const std::string &category) const {
  ScopedLock lock(
      &m_mutex); // FIXME 这个锁是可递归的吗，所以下面不能用getLogger("root") ?
...
LogIniter::LogIniter() {
  auto log_config_list = meha::Config::Lookup<std::vector<LogConfig>>(
      "logs", {}, "日志器的配置项");
