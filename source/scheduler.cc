...
Scheduler::Scheduler(size_t thread_size, bool use_caller, std::string name) : m_name(std::move(name))
{
    ...
    if (use_caller) {
        ...
        // 线程池需要的线程数减一
        --thread_size;
        ASSERT_FMT(GetThis() == nullptr, "线程下只有一个调度器！");
        t_scheduler = this;
        // 因为 Scheduler::run 是实例方法，需要用 std::bind 绑定调用者 //FIXME 为什么不用std::mem_fn
...
void Scheduler::start()
{
    ...
    {  // !!! 作用域锁
        ...
        for (size_t i = 0; i < m_thread_count; i++) {
            ...
            m_thread_id_list.push_back(m_thread_list[i]->GetCurrentId());
        }
    }
...
void Scheduler::stop()
{
    ...
    if (m_root_fiber && m_thread_count == 0 &&
        (m_root_fiber->isFinished() || m_root_fiber->getState() == Fiber::INIT)) {
        m_stopping = true;
        if (onStop())
            return;
    }
...
{
    //    LOG_DEBUG(root_logger, "调用 Scheduler::tickle()");
}
...
void Scheduler::run()
{
    ...
    if (GetThreadID() != m_root_thread_id) {  // 当前线程不存在 master fiber, 创建一个
        t_scheduler_fiber = Fiber::GetThis().get();
    }
    // 线程空闲时执行的协程
    auto idle_fiber = std::make_shared<Fiber>(std::bind(&Scheduler::onIdle, this));
    // 开始调度
    Task task;
    while (true) {
        task.reset();
        ...
        if (task.callback) {  // 如果是 callback 任务，为其创建 fiber
            task.fiber = std::make_shared<Fiber>(std::move(task.callback));
            task.callback = nullptr;
        }
        if (task.fiber && !task.fiber->isFinished()) {  // 是 fiber 任务
            ...
            } else if (fiber_status != Fiber::ERROR && fiber_status != Fiber::TERM) {
