...
Scheduler::Scheduler(size_t pool_size, bool use_caller)
{
    ...
    if (use_caller) {
        ...
        Thread::SetCurrentName(fmt::format("sche@{}/caller_worker", static_cast<void *>(this)));
...
void Scheduler::start()
{
    ...
    for (size_t i = 0; i < m_thread_pool_size; i++) {
        m_thread_pool.emplace_back(std::make_unique<Thread>(std::bind(&Scheduler::run, this), fmt::format("sche@{}/worker{}", static_cast<void *>(this), i)));
    }
}
...
{
    LOG_INFO(root_logger, "调用 Scheduler::tickle()");
}
...
void Scheduler::run()
{
    ...
    while (true) {
        ...
        if (task.handle && !(task.handle->isFinished())) {
            ...
            case Fiber::EXEC:
                // 如果换出时还是执行状态就抛异常
                throw RuntimeError(fmt::format("协程[{}]执行状态异常：当前状态 {}", task.handle->id(), static_cast<int>(task.handle->state())));
                break;
            }
        }
        // 任务队列空了，换入执行 idle_fiber，避免hang整个调度器
            ...
            case Fiber::TERM:
                ...
                LOG_FMT_DEBUG(root_logger, "idle协程[%ld]运行结束", idle_fiber->id());
                return;
            case Fiber::EXEC:
                // 如果换出时还是执行状态就抛异常
                throw RuntimeError(fmt::format("idle协程[{}]执行状态异常：当前状态 {}", task.handle->id(), static_cast<int>(task.handle->state())));
                break;
            }
        }
    }
