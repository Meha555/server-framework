...
void IOManager::idle()
{
    ...
    while (true) {
        if (isStoped()) {
            LOG_FMT_DEBUG(core, "调度器@%p 已停止执行", this);
            break;
        }
        ...
        for (int i = 0; i < result; i++) {
            ...
            if (ret == -1) {
                LOG_FMT_ERROR(core, "epoll_ctl(%d, %d, %d, %ul): return %d, errno %s(%d)", m_epoll_fd, op, fd_ctx->m_fd, ev.events, ret, ::strerror(errno), errno);
            }
            // 触发该 fd 对应的事件的处理器
