                    ...
                    meha::FdContext::FdEvent event, meha::FileDescriptor::TimeoutType fd_timeout_type, Args &&...args)
{
    ...
    if (func_name) {
        LOG_FMT_TRACE(core, "doIO 代理执行系统函数 %s", func_name);
    }
...
retry:
    ...
    if (n == -1 && errno == EAGAIN) {
        ...
        if (!rt) {
            if (func_name) {
                LOG_FMT_ERROR(core, "%s 添加事件监听失败(%d, %u)", func_name, fd, event);
            }
            if (timer) {
                timer->cancel();
            }
                       ...
                       socklen_t addrlen, uint64_t timeout_ms)
{
    ...
    if (rt == -1) {
        ...
        LOG_FMT_ERROR(core, "ConnectWithTimeout addEventListen(%d, WRITE) error", sockfd);
    }
    // 处理错误
    int error = 0;
...
// 这些用于侵入式Hook的函数都不能是static的（否则其他cpp文件中就用不了了）
