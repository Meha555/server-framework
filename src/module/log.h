...
#include "utils/utils.h" // 为了引入 utils namespace
...
#define LOG(category, level)                                                                                                                                                                         \
    if (GET_LOGGER(#category)->getLevel() <= meha::LogMessage::LogLevel::level)                                                                                                                      \
    meha::LogMessageWrapper(                                                                                                                                                                         \
        #category, std::make_shared<meha::LogMessage>(__FILE__, __LINE__, __PRETTY_FUNCTION__, meha::utils::GetThreadID(), meha::utils::GetFiberID(), #category, meha::LogMessage::LogLevel::level)) \
        .getSS()

// 使用C-style API打印日志的宏（使用默认格式）
#define LOG_LEVEL(category, level, message) \
    GET_LOGGER(#category)->log(MAKE_LOG_MSG(category, level, message))
#define LOG_TRACE(category, message) LOG_LEVEL(category, TRACE, message)
#define LOG_DEBUG(category, message) LOG_LEVEL(category, DEBUG, message)
#define LOG_INFO(category, message) LOG_LEVEL(category, INFO, message)
#define LOG_WARN(category, message) LOG_LEVEL(category, WARN, message)
#define LOG_ERROR(category, message) LOG_LEVEL(category, ERROR, message)
#define LOG_FATAL(category, message) LOG_LEVEL(category, FATAL, message)

// 使用C-style API的可设置格式打印日志的宏（这里额外加了一个大括号来增加作用域，放置内部定义的变量和外部冲突）
#define LOG_FMT_LEVEL(category, level, format, argv...)    \
    {                                                      \
        char *b = nullptr;                                 \
        int l = asprintf(&b, format, argv);                \
        if (l != -1) {                                     \
            LOG_LEVEL(category, level, std::string(b, l)); \
            free(b);                                       \
        }                                                  \
    }

#define LOG_FMT_TRACE(category, format, argv...) \
    LOG_FMT_LEVEL(category, TRACE, format, argv)
#define LOG_FMT_DEBUG(category, format, argv...) \
    LOG_FMT_LEVEL(category, DEBUG, format, argv)
#define LOG_FMT_INFO(category, format, argv...) \
    LOG_FMT_LEVEL(category, INFO, format, argv)
#define LOG_FMT_WARN(category, format, argv...) \
    LOG_FMT_LEVEL(category, WARN, format, argv)
#define LOG_FMT_ERROR(category, format, argv...) \
    LOG_FMT_LEVEL(category, ERROR, format, argv)
#define LOG_FMT_FATAL(category, format, argv...) \
    LOG_FMT_LEVEL(category, FATAL, format, argv)
...
{
    friend class internal::LogModule;
...
using LogConfigs = std::vector<LogConfig>;
...
{
public:
    LogMessageWrapper(const std::string &category, const LogMessage::sptr msg)
        : m_logger(GET_LOGGER(category))
        , m_message(msg)
    {
    }
    ~LogMessageWrapper()
...
// 应该是初始化顺序的问题，和hook冲突了
// static _LogIniter _;

// extern template struct lexical_cast<std::string, LogConfigs>;
// extern template struct lexical_cast<LogConfigs, std::string>;
